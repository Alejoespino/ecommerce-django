...
class RegistrationForm(forms.ModelForm):
    password =  forms.CharField(widget=forms.PasswordInput(attrs={
            'placeholder': 'ingrese paswword',
            'class':'form-control',
    }))
    confirm_password =  forms.CharField(widget=forms.PasswordInput(attrs={
            'placeholder': 'confirmar paswword',
            'class':'form-control', 
    }))
    class Meta:
        model = Account
...
class UserProfileForm(forms.ModelForm):
    profile_picture = forms.ImageField(required=False,error_messages={'invalid':('solo archivos de imagennn')}, widget=forms.FileInput)
    class Meta:
        model = UserProfile
